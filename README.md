...
Local file-backed `seek` and `live` items also show a framework-owned **Log** action immediately before the camera. It writes a timestamped text note into a `logs/` directory beside the source data. Each note records the UTC creation time, current playback position and mode, live-follow state, active tab and switched views, analysis parameters, page metadata, and workflow statistics. Existing notes appear as colored marks on the playback bar; hovering identifies the note and clicking its mark seeks to that saved position. A newly written note appears immediately. Plugins do not need to implement logging or choose a storage path; static and non-file-backed items omit the action.
