...
def main() -> None:
    try:
        ...
        raise SystemExit('Install build support first: pip install "workspace-browser[build]"') from exc

    arguments = sys.argv[1:] or ["--clean", "--noconfirm"]
    resource = files("workspace_browser._packaging").joinpath("workspace-browser.spec")
