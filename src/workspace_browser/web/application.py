...
function startFrameworkWindowed(config,refresh){
  ...
  const canvas=track.querySelector('canvas'),selection=track.querySelector('#windowed-selection'),left=track.querySelector('#windowed-left'),right=track.querySelector('#windowed-right'),startLabel=root.querySelector('#windowed-start-label'),endLabel=root.querySelector('#windowed-end-label');let drag=null,updating=false,pending=false;
  ...
  const render=()=>{clamp();const leftPercent=duration?windowStart/duration*100:0,rightPercent=duration?windowEnd/duration*100:100;selection.style.left=`${leftPercent}%`;selection.style.width=`${rightPercent-leftPercent}%`;left.style.left=`${leftPercent}%`;right.style.left=`${rightPercent}%`;startLabel.textContent=`${windowStart.toFixed(3)} s`;endLabel.textContent=`${windowEnd.toFixed(3)} s`;left.setAttribute('aria-valuenow',String(windowStart));right.setAttribute('aria-valuenow',String(windowEnd));drawOverview()};
  ...
  track.onpointerup=async event=>{if(!drag||drag.pointer!==event.pointerId)return;drag=null;track.releasePointerCapture(event.pointerId);await commit()};track.onpointercancel=()=>{drag=null};
