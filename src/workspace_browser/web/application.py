...
function startFrameworkWindowed(config,refresh){
  ...
  const editWidth=value=>{const parsed=Number(value);if(!Number.isFinite(parsed)||parsed<=0){render();return}const target=Math.min(duration,Math.max(minimum,parsed));windowEnd=windowStart+target;if(windowEnd>duration){windowEnd=duration;windowStart=Math.max(0,duration-target)}render();finalCommit()};
