...
function rememberPlotResetRanges(plot,view){const update={};for(const [name,axis] of Object.entries(view?.value?.layout||{})){if(!/^[xy]axis\d*$/.test(name)||!Array.isArray(axis?.range)||axis.range.length<2)continue;const range=axis.range.map(Number);if(range.every(Number.isFinite)){update[`${name}.range`]=range;update[`${name}.autorange`]=false}}plot._sigvueResetRanges=update}
...
function startFrameworkWindowed(config,refresh){
  ...
  const selectedOverview=()=>{const series=config.overview_series||[];if(!series.length)return config.overview_values||[];const selected=Number(viewSelections[config.overview_switcher_key]||0);return series[Math.min(Math.max(0,selected),series.length-1)]||config.overview_values||[]};
  ...
  redrawWindowOverview=drawOverview;
